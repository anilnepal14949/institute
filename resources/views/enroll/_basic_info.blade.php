...
<!-- dob -->
    <div class="form-group col-md-6 col-lg-6 @if($errors->has('dob')) has-error @elseif(count($errors->all())>0) has-success @endif has-feedback">
        <div class="input-group col-md-12 col-lg-12 showToolTip" @if($errors->has('dob')) title="{!!$errors->first('dob')!!}" @endif>
        {!!Form::label('dob','Date of Birth:')!!}
        <input type="date" name="dob" id="dob" class="form-control" max="{{date('Y-m-d')}}" @if(isset($userDetail)) value="{{$userDetail->dob}}" @endif />
        @if($errors->has('dob'))
            <span class="glyphicon glyphicon-remove-circle form-control-feedback" aria-hidden="true"></span>
            <span id="dobStatus" class="sr-only">(error)</span>
        @elseif(count($errors->all())>0)
            <span class="glyphicon glyphicon-ok-circle form-control-feedback" aria-hidden="true"></span>
            <span id="dobStatus" class="sr-only">(success)</span>
        @endif
    </div>
</div>
